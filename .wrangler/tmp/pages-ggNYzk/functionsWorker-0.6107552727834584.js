...
// v1/save.ts
var KEY_RE = /^[A-Za-z0-9_-]{16,80}$/;
var MAX_SAVE_BYTES = 3e5;
...
__name(json, "json");
function deviceKeyFrom(auth) {
  if (!auth?.startsWith("Bearer ")) return null;
  const key = auth.slice(7).trim();
  return KEY_RE.test(key) ? key : null;
}
__name(deviceKeyFrom, "deviceKeyFrom");
...
function acceptsWrite(stored, incoming) {
  return !stored || incoming.rev > stored.rev;
}
__name(acceptsWrite, "acceptsWrite");
async function readEnvelope(db, key) {
  const row = await db.prepare("SELECT rev, updated_at, save FROM saves WHERE device_key = ?").bind(key).first();
...
async function onRequestPut(ctx) {
  const key = deviceKeyFrom(ctx.request.headers.get("authorization"));
  if (!key) return json(401, { error: "missing or malformed device key" });
  let body;
  try {
  ...
  await ctx.env.DB.prepare(
    "INSERT INTO saves (device_key, rev, updated_at, save) VALUES (?, ?, ?, ?) ON CONFLICT(device_key) DO UPDATE SET rev = excluded.rev, updated_at = excluded.updated_at, save = excluded.save"
  ).bind(key, incoming.rev, incoming.updatedAt, incoming.save).run();
  return json(200, { ok: true, rev: incoming.rev });
}
__name(onRequestPut, "onRequestPut");

// ../.wrangler/tmp/pages-ggNYzk/functionsRoutes-0.17920606231474356.mjs
var routes = [
  {
    routePath: "/v1/save",
    mountPath: "/v1",
...
// ../node_modules/.pnpm/wrangler@4.108.0/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
