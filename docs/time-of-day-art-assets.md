...
- **UI theming** (panels, brass, buttons, energy pill shifting with the day) is done with
  **CSS variables**, not images — no assets. *(Shipped: `core/environment.ts` +
  `ui/environment-controller.ts` push ~9 `--env-*` vars — background, panels, button/
  progress warmth, nav, and the energy-pill glow — to `<html>` each tick; surfaces consume
  them with slow eases; `body.high-contrast` pins them; text/semantic/currency/focus are
  never themed.)*
